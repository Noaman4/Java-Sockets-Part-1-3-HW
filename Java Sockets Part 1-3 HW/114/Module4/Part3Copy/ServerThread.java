...
public class ServerThread extends Thread {
    ...
    private void processNumberGuesserCommand(String command) {
        if (command.equalsIgnoreCase("/start")) {
            server.broadcast("Number guesser game has started! Guess a number between 0 and 50.", -1);
            isNumberGuesserActive = true;
        } else if (command.equalsIgnoreCase("/stop")) {
            server.broadcast("Number guesser game has stopped. Guesses will now be treated as regular messages.", -1);
